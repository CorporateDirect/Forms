...
let navigationValidationResult: NavigationValidationResult | null = null;
 ...
 */
export function validateNavigationPattern(root: Document | Element = document): NavigationValidationResult {
  logVerbose('=== NAVIGATION PATTERN VALIDATION ===');
  
  const issues: NavigationIssue[] = [];
  const navigationMap: Record<string, NavigationElement[]> = {};
  ...
  const answerTargets = new Set<string>();
 ...
 */
function handleRadioClick(event: Event, target: Element): boolean {
  const htmlTarget = target as HTMLElement;
  let radioInput: HTMLInputElement | null = null;
  let clickContext: string | null = null;
  ...
  if (htmlTarget instanceof HTMLInputElement && htmlTarget.type === 'radio') {
    ...
    if (parentRadioContainer) {
      radioInput = parentRadioContainer.querySelector('input[type="radio"]') as HTMLInputElement;
  ...
  if (radioInput) {
    ...
    if (radioInput.name) {
      const groupRadios = document.querySelectorAll(`input[type="radio"][name="${radioInput.name}"]`) as NodeListOf<HTMLInputElement>;
 ...
 */
function updateRadioVisualState(radioInput: HTMLInputElement, isChecked: boolean): void {
  const label = radioInput.closest('label');
  const visualElements = label?.querySelectorAll('.w-radio-input, .radio_button-skip-step, .w-form-formradioinput, .form_radio-icon') as NodeListOf<HTMLElement>;
 ...
 */
function triggerNavigationEvents(radioInput: HTMLInputElement): void {
 ...
 */
function initializeRadioStates(root: Document | Element = document): void {
  const allRadios = root.querySelectorAll('input[type="radio"]') as NodeListOf<HTMLInputElement>;
 ...
 */
function setupEventListeners(root: Document | Element = document): void {
...
function createDebugInterface(): void {
  const debugInterface: NavigationLibrary = {
    validateNavigation: () => validateNavigationPattern(),
    debugRadios: () => {
      const allRadios = document.querySelectorAll('input[type="radio"]') as NodeListOf<HTMLInputElement>;
      const radioGroups: RadioDebugInfo = {};
  ...
  (window as any).FormNavigationLib = debugInterface;
 ...
 */
export function initNavigation(root: Document | Element = document): NavigationValidationResult {
  if (initialized) {
...
export function resetNavigation(): void {
  ...
  delete (window as any).FormNavigationLib;
...
export function getNavigationState(): any {
  return {
    ...
    hasDebugInterface: !!(window as any).FormNavigationLib
  };
}
 ...
 */
export function validateNavigationTarget(target: string): boolean {
 ...
 */
export function getNavigationIssues(): NavigationIssue[] {
